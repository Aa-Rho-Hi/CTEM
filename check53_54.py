...
async def main():
    ...
    async with get_session_factory()() as session:
        # Create finding
        v = Vulnerability(
            id=uuid4(), tenant_id=TENANT_ID, asset_id=None, cve_id="CVE-2024-4444",
            source_tool="test", port=None, risk_score=70, cvss_score=7.0,
            epss_score=0.7, false_positive_score=0.1, is_kev=False,
            status=FindingStatus.open, fingerprint_hash=f"boost-hash-{uuid4().hex[:16]}",
        ...
        # Create agent decision linked to this finding
        d = AgentDecision(
            agent_id=AGENT2_ID,
            tenant_id=TENANT_ID,
            goal=f"analyze finding {v.id}",
            reasoning_chain={"finding_id": str(v.id), "summary": "analyzed"},
        ...
        print(f"LINKED_FINDING_ID={v.id}")
        print(f"LINKED_DECISION_ID={d.id}")
